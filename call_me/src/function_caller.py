...
def load_vocab(model: Small_LLM_Model) -> dict[int, str]:
    """Load the vocabulary file from the model's vocab path.

    Parameters
    ----------
    model : Small_LLM_Model
        The language model instance used to retrieve the vocab file path.
    ...
    ------
    FileNotFoundError
        If the vocab file does not exist at the expected path.
    """
    ...
    sorted_vocab = dict(sorted(
        id_to_token.items(), key=lambda x: len(x[1]), reverse=True
    ))
...
def process_prompt(
    ...
    try:
        ...
        fn_descriptions = "\n".join(
            ...
            + ["- fn_no_function: use this if no function matches the request"]
        )
        ...
        name_prompt = (
            ...
            "If it is a question, greeting, or unrelated request, \
            return fn_no_function.\n\n"
        ...
        if not fn_name or fn_name not in valid_names:
            print(
                f"{ORANGE}[WARNING]{RESET} Could not select function for: \
                    {user_prompt!r}",
                file=sys.stderr,
            )
            return None
        ...
        if fn_def is None or fn_def.name == "fn_no_function":
            print(
                f"{ORANGE}[WARNING]{RESET} Function not found: {fn_name}",
                file=sys.stderr,
            )
            return None
...
def run_pipeline(
    ...
    for i, test_prompt in enumerate(prompts):
        print(
            f"{GREEN}[INFO]{RESET} {i + 1}/{len(prompts)}: {test_prompt.prompt!r}",  # noqa
            file=sys.stderr,
        )
